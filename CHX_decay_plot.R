...
if ("--help" %in% args || "-h" %in% args) {
  cat("Usage: Rscript CHX_decay_plot.R [gene_name] [--global]\n")
  cat("Arguments:\n")
  ...
  cat("Prerequisite:\n")
  cat("  Run CHX_p_value.R first to create HalfLife_AllProteins.xlsx.\n")
  q(save = "no")
}
...
if (nrow(fdr_match) != 1) {
  stop(sprintf(
    "Expected one FDR result for gene '%s' / protein group '%s'; found %d. Rerun CHX_p_value.R.",
